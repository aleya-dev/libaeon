...
TEST(test_hash, test_hash_combined)
{
    ...
    Common::HashCombine(hash, 0);
    EXPECT_NE(0, hash);
    EXPECT_EQ(Common::HashCombined(0), hash);

    const auto previous_value = hash;
    Common::HashCombine(hash, 0);
    EXPECT_NE(previous_value, hash);
    EXPECT_NE(0, hash);
    EXPECT_EQ(Common::HashCombined(0, 0), hash);
}
...
TEST(test_hash, test_hash_combined_lvalue)
{
    const auto value1 = 1;
    const auto value2 = 2;
    EXPECT_EQ(Common::HashCombined(1, 2), Common::HashCombined(value1, value2));
}
...
TEST(test_hash, test_variadic_hash_combine)
{
    ...
    hash = 0;
    Common::HashCombine(hash, 0, 0);
    const auto result2 = hash;

    hash = 0;
    Common::HashCombine(hash, 0, 0, 0);
