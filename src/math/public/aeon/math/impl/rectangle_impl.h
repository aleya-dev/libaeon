    ...
    -> rectangle<T>
{
    ...
    switch (anchor)
    {
        ...
        default:
        {
            AeonAssertFail("Unexpected anchor point.");
            return {};
        }
    }
}
...
{
    AeonAssertValueInRange(ratio, 0.0f, 1.0f);
...
inline constexpr auto slice_horizontal_absolute(const rectangle<T> &rect, const T value) noexcept
    -> std::tuple<rectangle<T>, rectangle<T>>
{
    AeonAssertValueInRange(value, top(rect), bottom(rect));
...
inline constexpr auto slice_vertical_absolute(const rectangle<T> &rect, const T value) noexcept
    -> std::tuple<rectangle<T>, rectangle<T>>
{
    AeonAssertValueInRange(value, left(rect), right(rect));
                                                   ...
                                                   const T tolerance) noexcept -> Common::Flags<rectangle_edge>
{
    ...
    edges.ConditionalSet(position.x >= left(rect) - tolerance && position.x <= left(rect) + tolerance,
                          rectangle_edge::left);
    edges.ConditionalSet(position.y >= top(rect) - tolerance && position.y <= top(rect) + tolerance,
                          rectangle_edge::top);
    edges.ConditionalSet(position.x >= right(rect) - tolerance && position.x <= right(rect) + tolerance,
                          rectangle_edge::right);
    edges.ConditionalSet(position.y >= bottom(rect) - tolerance && position.y <= bottom(rect) + tolerance,
