...
{
    return Common::String{Common::StringUtils::ReplaceCopy(path, "\\", "/").AsStdU8StringView()};
}

auto detail::is_image_extension(const Common::String &extension) -> bool
{
    const auto extension_lower = Common::StringUtils::ToLowerCopy(extension);
                                   ...
                                   const request &request)
{
    ...
    if (!Common::StringUtils::BeginsWith(full_path.u8string(), base_path_.u8string()))
    {
...
{
    const auto filename_str = Common::StringUtils::ToLowerCopy(filename);
